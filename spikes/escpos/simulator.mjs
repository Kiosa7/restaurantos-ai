// "Impresora" simulada: decodifica bytes ESC/POS de vuelta a comandos
// estructurados + un preview ASCII, para poder probar sin hardware físico.
// Esto NO reemplaza el spike de hardware real (impresora térmica 80mm) —
// ese sigue ⛔ bloqueado (ver mini-informe). Sirve para validar la CAPA DE
// SOFTWARE: secuencia de comandos, textos, y que cutPartial/openDrawer se
// emiten cuando deben.

export function simulate(bytes) {
  const cmds = [];
  const lines = [];
  let current = "";
  let i = 0;
  while (i < bytes.length) {
    ...
    if (b === 0x1b && bytes[i + 1] === 0x40) {
      ...
      cmds.push({ op: "align", value: ["left", "center", "right"][bytes[i + 2]] });
      i += 3;
